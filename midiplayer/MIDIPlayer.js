...
function MIDIPlayer(source,onload) {
    ...
    this.PAUSED = "paused";
    this.tempo = 1.0; // Default tempo multiplier (1.0 = normal speed)
    ...
    this.tick=function(song, stepDuration) {
        var adjustedStepDuration = stepDuration / this.tempo;
        if (audioContext.currentTime > nextStepTime - adjustedStepDuration) {
            ...
            nextStepTime = nextStepTime + adjustedStepDuration;
            if (currentSongTime > song.duration) {
                this.log("End of song, duration:", song.duration);
                if (this.onend && typeof(this.onend)=="function"){
                    this.onend(loadedsong);
                }
                if (this.autoReplay){
                    this.log("Looping: resetting to start");
                    // Reset to beginning immediately without time gap
                    currentSongTime = 0;
                    songStart = audioContext.currentTime;
                    nextStepTime = audioContext.currentTime;
                } else {
        ...
        for (var t = 0; t < song.tracks.length; t++) {
            ...
            for (var i = 0; i < track.notes.length; i++) {
                if (track.notes[i].when >= start && track.notes[i].when < end) {
                    var when = songStart + track.notes[i].when / this.tempo;
                    var duration = track.notes[i].duration / this.tempo;
                    if (duration > 3) {
                        duration = 3;
                    }
        ...
        for (var b = 0; b < song.beats.length; b++) {
            ...
            for (var i = 0; i < beat.notes.length; i++) {
                if (beat.notes[i].when >= start && beat.notes[i].when < end) {
                    var when = songStart + beat.notes[i].when / this.tempo;
                    var duration = 1.5 / this.tempo;
    ...
    this.setTempo=function(tempo){
        // tempo can be from 0.25 (25%) to 3.0 (300%)
        if (tempo < 0.25) tempo = 0.25;
        if (tempo > 3.0) tempo = 3.0;
        this.tempo = tempo;
        this.log("Tempo set to", tempo);
    }
    ...
    // Expose URL-based loading for external callers (e.g., index.html autoload)
